...
void* handle_server_decryption(char* word, char* file_name){
    ...
    // Check if the word is not empty before writing to the file
    if (word_len > 0) {
        char entry[256];
        ...
        printf("Writing decryption %s to file\n", entry);
    }
...
void* handle_server_encryption(char* word, int key, char* file_name){
    ...
    // Write the formatted data to the file
    fprintf(file, "%s", entry);
    printf("Writing encryption %s to file\n", entry);
...
void* decryptFile(char* file_name){
    ...
    while (fgets(line, sizeof(line), file) != NULL) {
        if (sscanf(line, "%s : %d", word, &number) == 2) {
            ...
            response = receive_RPC_response(socket);
            if (response) {
    ...
    request = create_RPC_Request(operation, NULL, -1);
    send_RPC_request(socket, request);
    close(socket);
    free(response);
    //return 0;
